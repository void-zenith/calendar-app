    ...
    <div className="describe-event__container">
      ...
      <div className="describe-event__body">
        <div className="from-to__container">
          ...
          <h2>
            TO: <span className="date">{eventDetails._instance.range.start.toDateString()}</span>
          </h2>
        </div>
        <div className="participation__container">
          <h2>Participation</h2>
          {eventDetails.extendedProps.Participation &&
            eventDetails.extendedProps.Participation.map((ev, id) => (
              <div className="partFlex" key={id}>
                <p className="paraFlex">{ev.part}</p>
              </div>
            ))}
        </div>
